...
import { scene, camera, renderer, controls } from './sceneCameraRendererControls.js'
...
function onClick(event) {
  ...
  if (intersects.length > 0) {
    ...
    if (country.userData.name && !prevZoom) {
      ...
      const targetPosition = intersectionPoint.normalize().multiplyScalar(150)
      ...
      const targetPosition = camera.position.clone().normalize().multiplyScalar(200)
